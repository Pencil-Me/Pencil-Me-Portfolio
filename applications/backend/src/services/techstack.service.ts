...
import {
  convertITechStackToTTechStack, convertITechStackTypesToTTechStackTypes,
  type ITechStack,
...
class TechStacksService {
  ...
  async getTechStack(id: string): Promise<ITechStack> {
    return await
      this.axiosInstance
        .get('techstack/' + id)
        .then((response: { data: ITechStack }) => {
          return convertITechStackToTTechStack(response.data)
        })
  }

  async createTechStack(payload: TTechStack) {
    const sendobject = convertITechStackToTTechStack(payload)
    return await
      this.axiosInstance
        .post('techstack', sendobject)
        .then((response) => {
  ...
  async updateTechStack(payload: TTechStack) {
    ...
    return await
      this.axiosInstance
        .put('techstack/' + payload.id, sendobject)
        .then((response) => {
          return response.data
        })
  }
